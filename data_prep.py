...
#Apply lets you specify a function and then apply it on every row. Lambda means it
...
cr_labevents = cr_labevents[(cr_labevents.category == 'Chemistry') & 
  (cr_labevents.fluid == 'Blood')][['hadm_id','ip_dates','valuenum','flag']].groupby(['hadm_id','ip_dates']
  ).agg(Creatinine = ('valuenum','max'), cr_flag = ('flag',lambda xx : max(np.where(xx == 'abnormal','1','0')))
  ).reset_index(drop = False).drop_duplicates()
...
admissions_scaffold['ip_dates'] = admissions_scaffold.apply(lambda row:pd.date_range(row['admittime'],
  row['dischtime'], freq = 'D'), axis = 1)
...
analysis_data = abx_dates.merge(cr_labevents, on = ['hadm_id','ip_dates'], how = 'left'
